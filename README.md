...
> **AWSでデプロイ状態の場合の注意点**
> 社員ログインの画面でログインや認証コードでログインしてアクセスする場合は、低い性能のAWSの「t3.micro」サーバーを使っている為、ログイン時にエラーが起きますが、２回~3回ログインを試さないとアクセスできませんので、ご注意ください。

---
## AttendEase(勤怠管理システム)の流れ
1.「http://localhost:3000/login」にアクセスすると「社員ログイン」の画面が、表示されます。
