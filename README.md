...
- [目的](#目的)
- [設計判断](#設計判断)
- [AttendEase(勤怠管理システム)の流れ](#attendease勤怠管理システムの流れ)
- [機能](#機能)
- [ロール](#ロール)
- [技術スタック](#技術スタック)
- [動作確認時の注意事項](#動作確認時の注意事項)
- [開発環境のセットアップ](#開発環境のセットアップ)
  - [必要なもの](#必要なもの)
  - [起動手順](#起動手順)
  - [テストユーザー](#テストユーザー)
  - [停止・削除](#停止削除)
- [本番環境（HTTPS化）](#本番環境https化)
- [CI/CD](#cicd)
  - [CI(`.github/workflows/ci.yml`)](#cigithubworkflowsciyml)
  - [CD(`.github/workflows/deploy.yml`)](#cdgithubworkflowsdeployyml)
- [画面遷移図](#画面遷移図)
- [画面モックアップ](#画面モックアップ)
  - [ログイン画面](#ログイン画面)
  - [パスワードリセット（2ステップ）](#パスワードリセット2ステップ)
  - [ダッシュボード（打刻カード）](#ダッシュボード打刻カード)
- [E-R 図](#e-r-図)
- [ブランチ運用ルール](#ブランチ運用ルール)
- [おまけ機能: CoDroneEDUコントローラーでの打刻連携](#おまけ機能-codroneeduコントローラーでの打刻連携)
  - [操作フロー図](#操作フロー図)
  - [Windowsでのセットアップ手順](#windowsでのセットアップ手順)
  - [操作方法](#操作方法)
  - [トラブルシューティング](#トラブルシューティング)

## 目的

業務アプリを設計・実装できるレベルにあることを示す題材として、「勤怠管理システム」を選びました。
打刻・承認フロー・権限管理など業務アプリに必要な要素が揃っており、実務に近い設計判断が求められるためです。

実装では「動くだけ」で終わらせず、以下を工夫しました。

- **ジオフェンス機能**による不正打刻の防止
- **残業時間の15分丸め処理**をロジック共通化で保守しやすく設計
- **アカウントロック解除**などの管理者機能でセキュリティと運用性を両立
- **CI/CD・HTTPS化**まで行い、実運用できる状態に仕上げた

## 設計判断

### ジオフェンス打刻制限

- **課題**: 出社/リモートの申告だけでは、会社にいないのに「出社」で打刻できてしまい、勤怠の信頼性が担保できない。
- **判断**:
  - 位置情報で登録拠点の半径内かを判定し、範囲外・位置情報が取得できない場合は打刻を拒否する。判定はバックエンドで行う。
  - 拠点の登録と機能のON/OFFはADMINがアプリ上で管理し、運用に合わせて切り替えられるようにする。
  - ON中は、申告した時刻を使わずサーバー時刻を打刻時刻にする。また、打刻修正・修正申請を禁止する。
  - 位置情報の取得にはHTTPSが必須なため、本番のHTTPS化と合わせて設計した。
- **結果**: 「拠点内で打刻した後に時刻だけ書き換える」という抜け道を塞ぎ、出社打刻の信頼性を担保した。

### 残業時間の15分丸め処理

- **課題**: 残業時間の丸めルールが、勤怠一覧とレポート出力の複数箇所に必要になる。
- **判断**: 「15分未満切り捨て」を `round_down_overtime_minutes()` の1関数に集約し、集計する4か所すべてから呼ぶようにした。
- **結果**: 丸め単位を変えるときの修正箇所が1つになり、画面とCSVで残業時間がずれない。

### アカウントロックと解除の監査ログ

- **課題**: ログインの失敗を放置すると総当たり攻撃を受けるが、ロックだけでは、正規の社員が使えなくなったときに困る。
- **判断**: 3回失敗でロックし(判定は `User.is_locked` に集約)、解除はADMINのみが行う。解除のたびに「誰が・誰を・いつ」をログに記録する。
- **結果**: セキュリティと運用性を両立し、解除の操作を後から追跡できる。

### CI/CDとHTTPS化

- **課題**: 手作業のデプロイは、ミスや確認漏れが起きやすい。
- **判断**:
  - GitHub Actionsで、PR時と`main`へのマージ時にテスト・lint・ビルドを自動実行する。`main`へのマージ後は、EC2へ自動でデプロイする。
  - デプロイ中だけ、実行環境のIPにSSH(22番ポート)を許可し、失敗した場合も含めて必ず許可を取り消す。
  - nginxとLet's Encryptの証明書でHTTPS化し、12時間ごとに自動更新する。
- **結果**: マージするだけで、HTTPSの環境へ安全に反映される。
...
2.一般社員:新規ログインする場合は、新規登録をクリックしてください。新規登録が完了する前に、必ず、「社員ID」と「パスワード」はメモするようにお願いします。新規アカウントに必要な「社員ID」、「氏名」、「メールアドレス」、「パスワード」を入力して、「登録」をクリックして完了するとログイン画面に戻り、新規アカウントの作成が完了しました。
...
⚠️既存のアカウントで、正しいユーザーIDとパスワードの入力に「３回失敗」すると「アカウントロック」になりますので、ご注意ください。
...
4.一般社員:受信した認証コードをフォームに入力して、ログインしてください。※もし、時間内に入力できない場合は、「再送信」をクリックして、認証コードのフォームに入力してください。
...
6.勤怠登録の「出勤」と「退勤」では、手動での入力もしくは、時計のアイコンをクリックして、数字を選択しての時間設定を行い、「出勤する」または「退勤する」をクリックしてください。※出勤時のみ、「出社」もしくは「リモート」を選択できます。
...
20.システム管理者:「拠点を追加」をクリックしてください。→「拠点名」、「緯度」、「経度」、「許容半径(メートル)」を入力し、「有効にする」を設定してください。※「↖️現在地」を使用することで、緯度と経度は、自動入力することができます。(位置情報を許可する必要があるので、「サイトへのアクセス時は、許可」もしくは、「今回のみ許可」どちらかをクリックしてください。)→「保存」をクリックしてください。
...
21.一般社員：システム管理者が、ジオフェンス機能を「ON」にしている場合のみ、GPSをONにして、なりすましの出勤と退勤を防ぐために、ジオフェンス機能で定めた範囲外での出勤もしくは、退勤での時間の入力ができない、修正もできない、勤怠一覧での前日の出勤と退勤での登録もできないようになっています。
...
23.システム管理者：一般社員などのユーザーが社員ログインで「アカウントロック」された場合、システム管理者が「ユーザー一覧」をクリックして、「ロック」と表記されたユーザーを探し、項目の「操作」にある「ロック解除」をクリックすると、アカウントロックが解除されます。
...
24.システム管理者:ユーザー一覧の操作項目の「履歴」で確認でき、アカウントロックの解除の時だけ操作の不正を防止するため、監査ログとして「いつ、誰が、何を」 というログを残せるようにしています。
...
| EMPLOYEE | 打刻・各種申請 |
| MANAGER | 部下の申請承認 |
| ADMIN | ユーザーアカウント管理・**ロールを指定したアカウント作成（ADMIN のみ可能）** |

> ログイン画面の「新規登録」から自己登録できるのは一般社員（EMPLOYEE）のみです。承認担当者（MANAGER）・システム管理者（ADMIN）のアカウントは、ADMIN がロールを指定して作成します。
...
> 時刻のドロップダウンメニューなどの表示が崩れる場合も、再読み込みで解消することがあります。
...
flowchart TD
  ...
  Toggle -->|"未出勤"| ClockIn["🎵🖼️ 出勤を記録\n(上昇メロディ+出勤アイコン表示)"]
  Toggle -->|"出勤中"| ClockOut["🎵🖼️ 退勤を記録\n(下降メロディ+退勤アイコン表示)"]
...
<img width="758" height="405" alt="スクリーンショット 2026-08-08 19 34 34" src="https://github.com/user-attachments/assets/7d2b7576-dc7b-4d94-b06d-1ca7484e12ed" />
...
出勤成功時は上昇するメロディとともにコントローラーのディスプレイに出勤アイコン(歩く人物のシルエット)が表示され、退勤成功時は下降するメロディとともに退勤アイコン(ドアから退室する人物のシルエット)が表示されます。通信エラーなど打刻に失敗した場合は、LEDが赤く点滅し低い音のエラー音が鳴ります。
