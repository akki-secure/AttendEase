...
> [!NOTE]
> AWS の低スペックインスタンス（t3.micro）を使用しているため、「次へ進む」をクリックした後にエラーが表示される場合があります。その場合は再度「次へ進む」をクリックしてください（サーバーの処理遅延によるもので、バグではありません）。
...
> [!NOTE]
> AWS の低スペックインスタンス（t3.micro）を使用しているため、OTP 認証コードの入力後に一度エラーが表示される場合があります。その場合は再度「認証コード」を入力してください（サーバーの処理遅延によるもので、バグではありません）。
